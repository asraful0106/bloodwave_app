...
export const MyBloodRequestsSection = ({
  ...
  const handleShare = async (token: string) => {
    await Clipboard.setStringAsync(`https://bloodapp.com/request/${token}`);
    ...
    >
          ...
          {FILTER_TABS.map((tab, index) => {
            const cnt =
              tab.key === "ALL"
                ...
                : localRequests.filter((r) => r.status === tab.key).length;
      ...
      {filtered.map((req, index) => {
        ...
        const urg = URGENCY_CFG[req.urgency_level] ?? {
          color: "#9E9E9E",
          label: req.urgency_level ?? "Unknown",
        };
        const progress =
          req.units_required > 0 ? req.qty / req.units_required : 0;
            ...
            <View style={{ padding: moderateScale(12) }}>
              ...
              {deadlinePast && (
                ...
                >
                  ...
                  >
                    Deadline has passed — consider updating or closing this
                    request.
                  </StyledText>
                </View>
              )}
              ...
              >
                ...
                >
                  <View
                    style={{
                      height: "100%",
                      width: `${progress * 100}%`,
                      backgroundColor: progress >= 1 ? "#2eb97b" : cfg.color,
                      borderRadius: 3,
                    }}
                  />
                </View>
                ...
                >
                  {req.qty}/{req.units_required} units
                </StyledText>
              </View>
                ...
                {req.donors_count > 0 && (
                  <View
                    style={{
                      ...
                      alignItems: "center",
                      gap: moderateScale(3),
                      backgroundColor: withOpacity("#2196F3", 0.08),
                      borderRadius: moderateScale(7),
                      paddingHorizontal: moderateScale(6),
                      paddingVertical: moderateScale(3),
                    }}
                  >
                    ...
                    >
                      {req.donors_count} donor{req.donors_count > 1 ? "s" : ""}
                    </StyledText>
                  </View>
                )}
              </View>
                  ...
                  >
                    {req.status === "OPEN" && (
                      <>
                        {req.share_token && (
                          <TouchableOpacity
                            onPress={() => handleShare(req.share_token!)}
                            style={{
                              flexDirection: "row",
                              alignItems: "center",
                              gap: moderateScale(5),
                        ...
                        <TouchableOpacity
                          ...
                          style={{
                            ...
                            paddingHorizontal: moderateScale(12),
                            borderWidth: 1,
                            borderColor: withOpacity("#2eb97b", 0.3),
                          }}
                        >
                          <Feather
                            name="check-circle"
                    ...
                    {req.status === "FULFILLED" && req.donors_count > 0 && (
                      ...
                      >
                        ...
                        >
                          Thank {req.donors_count} donor
                          {req.donors_count > 1 ? "s" : ""}
                        </StyledText>
                      </View>
                    )}
                  </View>
