...
>((props, ref) => {
  const {
    ...
    mode = "normal", // NEW (default keeps old behavior)
    backgroundColor,
  ...
  // --- RIGHT ICON LOGIC (keeps old usage, but password mode toggles) ---
  const rightBase = rightIcon?.show === false ? undefined : rightIcon;

  const right =
    mode === "password"
      ? {
          // if user provided a rightIcon, we can keep its size/color,
          // but we override icon + onPress to toggle visibility
      ...
      >
        ...
        <View style={{ flex: 1, position: "relative" }}>
          ...
          <TextInput
            ...
            secureTextEntry={mode === "password" && !passwordVisible} // NEW
            onFocus={(e) => {
              setFocused(true);
